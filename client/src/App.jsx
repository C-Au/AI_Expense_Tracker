...
export default function App() {
  ...
  const handleCategoryChange = async (id, newCategory) => {
    try {
      const res = await axios.patch(`/api/expenses/${id}`, {
  ...
  const handleExport = async () => {
    ...
    sortedCategories.forEach((cat) => {
      ...
      group.forEach((e, j) => {
        lines.push(
          formatRow(e, j === group.length - 1 ? categoryTotal.toFixed(2) : ""),
        );
      });
      lines.push("");
    });
    ...
    if (typeof window.showSaveFilePicker === "function") {
      try {
        ...
        if (err.name === "AbortError") {
          ...
          setError(
            "Cannot save: the file is open in another application. Close it and try again.",
          );
        } else {
        ...
        <div className="error-banner">
          ...
          {csvParseError && (
            <span className="error-banner-hint">
              Please reformat your CSV to have 3 columns:{" "}
