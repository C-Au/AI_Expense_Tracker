...
router.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ error: parseErr.message, csvParseError: true });
    }
    if (parsed.length === 0) {
      return res
        .status(400)
    ...
    const customCats = await CustomCategory.find(
      {},
      { name: 1, _id: 0 },
    ).lean();
    const customCategoryNames = customCats.map((c) => c.name);

    const categorized = await categorizeExpenses(
      parsed,
      userRules,
      customCategoryNames,
    );
    ...
    const docs = categorized.map((exp) => ({
      ...exp,
      uploadBatch: batchId,
      userId: req.user.uid,
    }));
    ...
    res
      .status(201)
      .json({ uploadBatch: batchId, count: saved.length, expenses: saved });
  } catch (err) {
...
router.delete("/by-month/:month", async (req, res) => {
  try {
    ...
    res.json({
      message: `Deleted ${result.deletedCount} expense(s) for ${month}`,
...
router.get("/rules", async (req, res) => {
  try {
    const rules = await CategoryRule.find({ userId: req.user.uid }).sort({
      updatedAt: -1,
    });
    res.json(rules);
  } catch (err) {
